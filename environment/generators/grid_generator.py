...
class GridGenerator(Generator):
    ...
    def generate(self) -> Location:
        ...
        potential_locations = set([(middle + direction[0], middle + direction[1]) for direction in relative_directions])
        for i in range(self.number_of_locations):
            ...
            for direction in relative_directions:
                if not(0 <= new_position[0] + direction[0] < rows and 0 <= new_position[1] + direction[1] < columns):
                    continue
                if self.field[new_position[0] + direction[0]][new_position[1] + direction[1]] is None:
                    potential_locations.add((new_position[0] + direction[0], new_position[1] + direction[1]))
                else:
                    world_direction = ""
                    ...
                    Location.connect(new_locations, self.field[new_position[0] + direction[0]][new_position[1] + direction[1]], world_direction)
